...
int checkBuiltInCommands()
{
       ...
       /* if (strcmp("in", commandArgv[0]) == 0) {
        ...
        if (strcmp("f", commandArgv[0]) == 0)
        {
                if (commandArgv[1] == NULL)
                        return 0;
                 
                    printf("Ici on avance de %i cm",atoi(commandArgv[1]));
                 
                return 1;
        }
        if (strcmp("b", commandArgv[0]) == 0)
        {
                if (commandArgv[1] == NULL)
                        return 0;
                 
                    printf("Ici on recule de %i cm",atoi(commandArgv[1]));
                 
                return 1;
        }
	if (strcmp("fwd", commandArgv[0]) == 0)
        {

		//Front
		/*int semvalue;
		sem_getvalue(&buflockempty, &semvalue);

		printf("la semaphore empty est: %i \n", semvalue);*/
                sem_wait(&buflockempty);
		/*sem_getvalue(&buflockempty, &semvalue);
		...
		Commandbuffer.SpeedLeft = 9000;
		Commandbuffer.SpeedRight = 9000;
		...
		/*sem_getvalue(&buflockfull, &semvalue);

		printf("la semaphore full est: %i \n", semvalue);*/
		pthread_mutex_unlock(&mutex);
        	sem_post(&buflockfull);
		printf("la commande est fwd \n");
		/*sem_getvalue(&buflockfull, &semvalue);
		...
		Commandbuffer.type = CONSIGNESANSREPLIQUES;
      		...
      		sem_post(&buflockfull);
                    ...
                    printf("Ici on avance pour toujours \n");
